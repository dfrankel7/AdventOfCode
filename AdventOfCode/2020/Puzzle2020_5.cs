...
namespace AdventOfCode
{
    public class Puzzle2020_5
    {
        ...
        public static int FindOutSeat()
        {
            int ourSeat = 0;
            HashSet<int> filledSeats = new HashSet<int>();

            for (int i = 0; i < _input.Count; i++)
            {
                filledSeats.Add(ProcessSeat(_input[i]));
            }

            for (int j = 1; j < HIGHEST_SEAT + 1; j++)
            {
                if (!filledSeats.Contains(j) && filledSeats.Contains(j-1) && filledSeats.Contains(j+1))
                {
                    ourSeat = j;
                    break;
                }
        ...
        public static int ProcessSeat(string seatIdentifier)
        {
            int row = 0;
            int seat = 0;
            int lowerBound = 0;
            int upperBound = MAX_ROWS - 1;
            string rowIndicator = seatIdentifier.Substring(0, 7);
            string seatIndicator = seatIdentifier.Substring(7);

            for (int i = 0; i < rowIndicator.Length - 1; i++)
            {
                switch (rowIndicator[i])
                {
                    case 'F':
                        upperBound = (int)Math.Floor((upperBound + lowerBound) / 2f);
                        break;
                    case 'B':
                        lowerBound = (int)Math.Ceiling((upperBound + lowerBound) / 2f);
                        break;
                }
            }

            if (rowIndicator[rowIndicator.Length - 1] == 'F')
            {
                row = lowerBound;
            }
            else
            ...
            lowerBound = 0;
            upperBound = MAX_SEATS_PER_ROW - 1;

            for (int i = 0; i < seatIndicator.Length - 1; i++)
            {
                switch (seatIndicator[i])
                {
                    case 'L':
                        upperBound = (int)Math.Floor((upperBound + lowerBound) / 2f);
                        break;
            ...
            if (seatIndicator[seatIndicator.Length - 1] == 'L')
            {
                seat = lowerBound;
            }
            else
